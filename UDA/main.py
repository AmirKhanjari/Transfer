...
def load_data(args):
    ...
    '''
    source_loader, source_n_class = data_loader.load_data(
        data_loader.DATA_DIR2, args.batch_size, train=True, 
        num_workers=args.num_workers, 
        infinite_data_loader=not args.epoch_based_training
    )
    target_train_loader, _ = data_loader.load_data(
        data_loader.DATA_DIR1, args.batch_size, train=True, 
        num_workers=args.num_workers, 
        infinite_data_loader=not args.epoch_based_training
    )
    target_test_loader, _ = data_loader.load_data(
        data_loader.DATA_DIR1, args.batch_size, train=False, 
        num_workers=args.num_workers, 
        infinite_data_loader=False
    )
    return source_loader, target_train_loader, target_test_loader, source_n_class
